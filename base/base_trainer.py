...
class BaseTrainer:
    def __init__(self, config, model, criterion, train_loader, validate_loader, metric_cls, post_process=None):
        ...
        # device
        if paddle.device.cuda.device_count() > 0 and paddle.device.is_compiled_with_cuda():
            self.with_cuda = True
    ...
    def _initialize_optimizer(self):
        self.optimizer = self._initialize('optimizer', paddle.optimizer, parameters=self.model.parameters(), learning_rate=self.lr_scheduler)
