...
class BasicBlock(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=None):
        ...
        if not self.with_dcn:
            ...
            self.conv2 = DeformConv2D(planes, planes, kernel_size=3, padding=1, bias_attr=False)
...
class Bottleneck(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=None):
        ...
        if not self.with_dcn:
            ...
            self.conv2 = DeformConv2D(planes, planes, kernel_size=3, padding=1, stride=stride, bias_attr=False)
        self.bn2 = BatchNorm2d(planes, momentum=0.1)
...
def resnet18(pretrained=True, **kwargs):
    ...
    if pretrained:
        assert kwargs.get('in_channels', 3) == 3, 'in_channels must be 3 whem pretrained is True'
